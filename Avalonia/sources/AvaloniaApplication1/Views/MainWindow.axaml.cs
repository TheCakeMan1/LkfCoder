...
public partial class MainWindow : Window
{
    ...
    void Start(object sender, RoutedEventArgs e)
    {
        if (ch.IsChecked == true)
        {
            ...
            TextRes.Text = "Начинаю. Я могу зависнуть, это нормально...";
            string path = Folders1.Text;
            int text = new DirectoryInfo(path).GetFileSystemInfos("*.lkf").Length;
            string[] files = Directory.Exists(path) ? Directory.GetFiles(path, "*.lkf") : new string[0];
            w = text;
            Thread t1 = new Thread(new ThreadStart(Thread1ProcBack));
            Thread t2 = new Thread(new ThreadStart(Thread2ProcBack));
            Thread t3 = new Thread(new ThreadStart(Thread3ProcBack));
            Thread t4 = new Thread(new ThreadStart(Thread4ProcBack));
            switch (CheckBoxs)
            {
                case true:
                    switch (coreCount)
                    {
                        case 1:
                            TextRes.Text = "Начинаю. Я могу зависнуть, это нормально...";
                            w1 = w;
                            text = new DirectoryInfo(path).GetFileSystemInfos("*.mp3").Length;
                            Directory.CreateDirectory($"{Folders2.Text}/Back");
                            list = files;
                            t1.Start();
                            t1.Join();
                            break;
    ...
    private void Visable(object sender, RoutedEventArgs e)
    {
        nom.IsEnabled = true;
        Nomers.IsEnabled = true;
        Authors.IsEnabled = true;
        Titles.IsEnabled = true;
    }
    private void Unvisable(object sender, RoutedEventArgs e)
    {
        nom.IsEnabled = false;
        Nomers.IsEnabled = false;
    ...
    private void Thread1ProcBack()
    {
        for (int j = 0; j < w1; j++)
        {
            this.ConvertFile(list[j]);
            list[j] = list[j].Replace(".lkf", ".mp3");
            File.Move(list[j], $"{Folders2.Text}/Back/{Path.GetFileName(list[j])}");
        }
    }
    private void Thread2ProcBack()
    {
        for (int j = w1; j < w2; j++)
        {
            this.ConvertFile(list[j]);
    ...
    private void ConvertFile(string inputFileName)
    {
        string path = inputFileName.Replace(".lkf", ".mp3");
        BinaryReader reader = new BinaryReader(File.Open(inputFileName, FileMode.Open));
        BinaryWriter writer = new BinaryWriter(File.Open(path, FileMode.Create));
        FileInfo info = new FileInfo(inputFileName);
        uint[] numArray = new uint[0x80];
        for (int i = 0; i < ((info.Length / ((int)numArray.Length)) / 4); i++)
        {
            ...
            for (uint m = 3; m > 0; m--)
            {
                uint num = this.CalcKey(numArray[numArray.Length - 2], numArray[0], this.CalcLocal3(m), (uint)(numArray.Length - 1));
                numArray[numArray.Length - 1] -= num;
                for (int num5 = numArray.Length - 2; num5 > 0; num5--)
                {
                    num = this.CalcKey(numArray[num5 - 1], numArray[num5 + 1], this.CalcLocal3(m), (uint)num5);
                    numArray[num5] -= num;
                }
                num = this.CalcKey(numArray[numArray.Length - 1], numArray[1], this.CalcLocal3(m), 0);
                numArray[0] -= num;
            }
            for (int n = 0; n < numArray.Length; n++)
            {
